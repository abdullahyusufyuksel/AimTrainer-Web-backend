...
class AimTrainerDB
{
    ...
    async getLeaderBoard(gameType)
    {
        ...
        await this.dao.all(`SELECT DISTINCT games.player, MAX(games.points) as games.points, users.uuid FROM (games INNER JOIN users ON games.player = users.username) WHERE games.type=\"${gameType}\" GROUP BY player ORDER BY games.points DESC`).then(async(leaderboard)=>
        {
